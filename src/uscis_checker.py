...
An automated tool to check USCIS case status with:
...
async def get_flaresolverr_cookies() -> tuple:
    """
    Get Cloudflare clearance cookies from FlareSolverr.

    Returns:
        Tuple of (cookies list, user_agent string)

    Raises:
        Exception: If FlareSolverr request fails
    """
    async with aiohttp.ClientSession() as session:
        payload = {
            "cmd": "request.get",
        ...
        return result["solution"]["cookies"], result["solution"]["userAgent"]
...
async def check_case_status() -> Dict[str, Any]:
    """
    Main function to check USCIS case status.

    Uses FlareSolverr to bypass Cloudflare, then Playwright to interact
    with the USCIS website and extract status information.
    ...
    """
    # Step 1: Get Cloudflare clearance cookies
    print(f"[{datetime.now()}] Getting Cloudflare clearance via FlareSolverr...")
    cookies, user_agent = await get_flaresolverr_cookies()
    print(f"[{datetime.now()}] Got clearance cookies, now using Playwright...")

    # Step 2: Use Playwright with the cookies to interact with the page
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.goto(
                "https://egov.uscis.gov/casestatus/landing.do", timeout=30000
            )
            ...
            if not input_field:
                await page.screenshot(path="/app/data/debug_landing.png")
            ...
            await page.screenshot(path="/app/data/status_page.png")
